...
/**
 * This is an example class to show case-by-case API integration test usage:
 * I don't take credit for writing this...
 * Reference(s): 
 * http://ergast.com/mrd/ - ergast
 ...
 *
 */
public class ExampleRaceCarCircuitTest {
	...
	public void test_APIWithBasicAuthentication_ShouldBeGivenAccess() {
		given().auth().preemptive().basic("username", "password").when().get("http://path.to/basic/secured/api").then()
	...
	public void test_NumberOfCircuits_ShouldBe20_UsingResponseSpec() {
		given().when().get("http://ergast.com/api/f1/2017/circuits.json").then().assertThat()
				.spec(checkStatusCodeAndContentType).and().body("MRData.CircuitTable.Circuits.circuitId", hasSize(20));
	}
}
